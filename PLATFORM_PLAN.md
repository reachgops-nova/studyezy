...
**Where we are, in one line:** **Live in production** at https://studyezy-production.up.railway.app with all four roadmap features built (In-Unit Micro-Checks, Reasoning Interview, Prep Planner, Written Exam Capture & Coaching), a full rebrand (kangaroo mascot, navy/orange/warm-cream palette), an admin role, a public landing page, a voice picker, and a Groq (open-weight-model) fallback tier for Voice Q&A and Micro-Check reactions. Every feature verified end-to-end against a real click-through, not just typechecked.

**Standing workflow (agreed 2026-08-06):** every code change gets committed and pushed to GitHub automatically as part of doing the work — no need to ask each time. Push credentials are cached in this Mac's Keychain (see changelog, 2026-08-05 night entry) so this happens without friction. Railway CLI is also authenticated on this machine now (`railway login`, done 2026-08-13) so deploy-side changes (env vars, migrations, volumes) happen the same way — no need to ask each time either.
...
- `GROQ_API_KEY` — user is registering for a free-tier key at console.groq.com; once set on Railway, Voice Q&A gets a genuinely-dynamic fallback tier (before dropping to the local pattern-matcher) and Micro-Checks get real content-aware reactions instead of canned acks (before dropping to an honest generic one). Both already work today without it, just with less nuance.
- Everything else from earlier entries is resolved — see the changelog for details, including a real production build failure found and fixed today (Turbopack, not a code bug - see changelog)

**Up next:** CI + Playwright E2E tests for the deterministic flows (auth, navigation, content - AI-dependent flows still need the credit top-up to test meaningfully). Payment plans explicitly deferred until later, per the user. Content (Units 2-9, filling in 1.4-1.13) is otherwise the main remaining gap before this is a full product, not just a Unit-1 pilot.
