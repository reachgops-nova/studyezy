...
- Anthropic account credit top-up — this is now the single blocker standing between "code is correct" and "a real student gets real answers." Voice Q&A, grading, extraction, Reasoning Interview classification, and Exam Coaching all currently return the same graceful fallback message. Prep Planner is the one feature that's fully live regardless, since it's pure data aggregation with no Claude call.
- Everything else from earlier entries is resolved — see the changelog for details
