...
**Where we are, in one line:** **Live in production** at https://studyezy-production.up.railway.app with all four roadmap features built (In-Unit Micro-Checks, Reasoning Interview, Prep Planner, Written Exam Capture & Coaching), a full rebrand (kangaroo mascot, navy/orange/warm-cream palette), an admin role, and a public landing page. Every feature verified end-to-end against a real click-through, not just typechecked.
...
- Anthropic account credit top-up — the single blocker standing between "code is correct" and "a real student gets real answers" for grading, extraction, Reasoning Interview classification, and Exam Coaching (all currently return the same graceful fallback message). Voice Q&A no longer needs this at all - it has a working local fallback. Prep Planner never needed it either.
