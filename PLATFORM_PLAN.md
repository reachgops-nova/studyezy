...
**Where we are:** Opening a unit now starts with a **Unit Overview** (objectives in kid-facing language, pulled from the book's own "What can you do?" checklist, plus a scanned-pages section) and an optional **quick diagnostic check** ("how much do you already know?") before any teaching happens — this is the "brush up and skip" insight from the very first conversation, now actually implemented at concept granularity instead of jumping straight into content. After the diagnostic, each concept gets a per-concept recommendation ("You've got this" vs. "Review this"), routing straight into the avatar-led lesson only for what's weak. The lesson itself ("Ezy" the fox mascot) teaches via narrated chat bubbles, then asks a check-in question with quick-reply chips plus free voice/text input. Sample illustrations (original SVG) and a "video coming soon" slot are in place per concept. `ANTHROPIC_API_KEY` is configured (confirmed authenticating correctly) but the account has no credit yet, so live Q&A/grading answers aren't testable - fallback paths are confirmed working everywhere they're needed (voice Q&A, diagnostic short-answer grading). Build/lint/tests pass. Dev server has been running locally at localhost:3000 for the user to test directly.

**Still missing on the Unit Overview:** actual scanned textbook page images - we don't have saved files for the pages shared earlier in chat (image attachments aren't saved to disk automatically). Waiting on the user to drop image files into `public/content/igcse/stage5/english/unit-1/`.

**What we're trying to achieve right now:** Validate the full pilot loop (overview → diagnostic → per-concept recommendation → avatar-taught lesson → progression test → dashboard) with 1–3 real kids on Cambridge Stage 5 English, Unit 1 — currently only concepts 1.1–1.3 are fully built; 1.4–1.13 are stubbed as "coming soon."
...
- Actual scanned page image files (see above)
