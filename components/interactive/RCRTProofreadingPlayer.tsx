...
export interface RCRTProofreadingPlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'What is the main goal of proofreading a piece of writing?',
    options: [
      'To carefully check for errors in capitals, punctuation, spelling, and grammar before finishing',
      'To make the story as long as possible by repeating words',
      'To change all words into capital letters'
    ],
    correctIndex: 0,
    hint: 'Proofreading is the final inspection step to catch and fix spelling, capital letter, and punctuation mistakes!'
  },
  {
    id: 2,
    questionText: 'Why is reading your writing ALOUD a helpful proofreading strategy?',
    options: [
      'It helps you hear awkward phrasing, missing words, or misplaced capital letters',
      'It makes you read twice as fast',
      'It changes the story genre'
    ],
    correctIndex: 0,
    hint: 'Your ears often catch grammar and flow errors that your eyes skip over!'
  },
  {
    id: 3,
    questionText: 'Which items belong on a standard proofreading checklist?',
    options: [
      'Capital letters, end punctuation, spacing, spelling, and grammar',
      'Font color, paper weight, and pencil length',
      'Author salary and publisher phone number'
    ],
    correctIndex: 0,
    hint: 'Check for capitals, full stops, clear word spacing, spelling, and correct grammar!'
  }
];
...
const SENTENCE_ERROR_WORDS: WordError[] = [
  { id: '1', text: 'the', correctedText: 'The', isError: true },
  { id: '2', text: 'boy', correctedText: 'boy', isError: false },
  { id: '3', text: 'Ran', correctedText: 'ran', isError: true },
  { id: '4', text: 'to', correctedText: 'to', isError: false },
  { id: '5', text: 'School.', correctedText: 'school.', isError: true }
];

export const RCRTProofreadingPlayer: React.FC<RCRTProofreadingPlayerProps> = ({
  conceptName = 'Concept 1.6: Proofreading Checklist & Error Spotting',
  ...
  // Interactive Test State (Error Spotter)
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. Proofreading means carefully checking writing for errors in capital letters, end punctuation, word spacing, spelling, and grammar. Reading aloud helps your ears catch mistakes! For example, "the boy Ran to School" has capital letter errors that need fixing.'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Remember the checklist: Capitals, Punctuation, Spacing, Spelling, and Grammar.'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "Check capitals, full stops, spelling, and read aloud to catch errors!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your proofreader eyes on! Tap the 3 capital letter errors in "the boy Ran to School." to fix them!'
        );
      }
      ...
      safeNarrate(
        'Congratulations! You mastered Proofreading Checklist with RCRT active recall and earned 3 stars!'
      );
    }
  ...
  const handleFixWord = (item: WordError) => {
    if (!item.isError || fixedErrors.includes(item.id)) return;

    const nextFixed = [...fixedErrors, item.id];
    setFixedErrors(nextFixed);
    safeNarrate(`Error fixed: "${item.text}" ➔ "${item.correctedText}"!`);

    if (nextFixed.length === 3) {
      setTestCompleted(true);
      setStars((prev) => prev + 1);
      safeNarrate('All 3 errors fixed! Corrected sentence: "The boy ran to school." Proofreading recall 100% accurate!');
    }
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                <h3 className="font-bold text-base text-[#16241f] mb-1">
                  Rule: Proofreading Checklist
                </h3>
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>Capital Letters:</strong> Start sentences and proper nouns with capitals.<br/>
                  • <strong>End Punctuation:</strong> Full stops, question marks, or exclamation marks.<br/>
                  • <strong>Read Aloud:</strong> Catch spelling, grammar, and flow mistakes with your ears!
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  ✍️ Checklist Rule: "the boy Ran to School." ➔ "The boy ran to school."
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Lock the proofreading checklist (Capitals, Punctuation, Spelling, Grammar) in memory.
              </p>
            ...
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border-2 border-dashed border-[#9c6f1f]/40">
                <p className="text-sm font-black text-[#9c6f1f]">
                  "Check capitals, full stops, spelling, and read aloud to catch errors!"
                </p>
              </div>
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('Check capitals, full stops, spelling, and read aloud to catch errors!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active Error Spotter Test 🚀 ➔
              </button>
            </div>
          )}

          {/* STEP 4: TEST (HANDS-ON TAP-TO-FIX ERRORS) */}
          {rcrtStep === 'TEST' && (
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#9c6f1f] uppercase tracking-wider">
                  🎯 Step 4: Spot & Fix Sentence Errors
                </span>
                <span className="text-xs font-bold text-[#16241f]">
                  Errors Fixed: {fixedErrors.length} / 3
                </span>
              </div>
              ...
              <div className="p-5 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10 text-center space-y-3">
                <p className="text-xs text-[#16241f]/70 font-semibold">
                  Tap the <strong className="text-[#9c6f1f]">3 error words</strong> below to fix capital letter mistakes:
                </p>
                ...
                <div className="flex flex-wrap justify-center items-center gap-2 p-4 bg-white rounded-xl border border-[#16241f]/10 shadow-inner">
                  {SENTENCE_ERROR_WORDS.map((item) => {
                    const isFixed = fixedErrors.includes(item.id);
                    return (
                      <button
                        key={item.id}
                        onClick={() => handleFixWord(item)}
                        className={`py-2 px-3.5 rounded-lg font-mono font-bold text-base transition-all ${
                          isFixed
                            ? 'bg-emerald-600 text-white shadow-md scale-105'
                            : item.isError
                            ? 'bg-amber-100 text-amber-900 border-2 border-amber-400 hover:bg-amber-200'
                            : 'bg-gray-100 text-[#16241f]'
                        }`}
                      >
                        {isFixed ? item.correctedText : item.text}
                      </button>
                    );
                  })}
                </div>
                ...
                <div className="p-3 bg-white rounded-xl border border-[#16241f]/10 text-xs font-bold text-[#16241f]">
                  Corrected Preview: <span className="text-[#9c6f1f] font-mono text-sm">{fixedErrors.length === 3 ? 'The boy ran to school.' : 'The boy ran to...'}</span>
                </div>
              </div>
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 All 3 Errors Fixed! "The boy ran to school." Proofreader active recall 100% accurate!
                </div>
              )}
      ...
      {phase === 'passed' && (
        <div className="text-center py-8 space-y-5 animate-fade-in">
          ...
          <p className="text-xs text-[#16241f]/80 max-w-md mx-auto leading-relaxed">
            By Reading, Covering, Reciting, and Testing without looking, you built permanent memory for the Proofreading Checklist!
          </p>
...
export default RCRTProofreadingPlayer;
