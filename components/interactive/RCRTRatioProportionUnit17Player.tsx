...
export interface RCRTRatioProportionUnit17PlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'What is the main mathematical difference between ratio and proportion?',
    options: [
      'Ratio compares part-to-part (2:3), while proportion compares part-to-whole (2/5 or 40%)',
      'Ratio is for time, while proportion is for money',
      'They are two names for the exact same calculation'
    ],
    correctIndex: 0,
    hint: 'Ratio compares part to part using a colon; proportion compares part to the whole total as a fraction or percentage!'
  },
  {
    id: 2,
    questionText: 'In a tile pattern with 2 red tiles for every 3 blue tiles, what is the ratio of red to blue tiles?',
    options: [
    ...
    correctIndex: 0,
    hint: 'Order matters in a ratio! Red to blue is 2:3.'
  },
  {
    id: 3,
    questionText: 'In that same 5-tile pattern (2 red, 3 blue), what is the proportion of red tiles out of the whole?',
    options: [
      '2/5 or 40%',
      '2/3 or 66%',
      '3/5 or 60%'
    ...
    correctIndex: 0,
    hint: 'Proportion is part-to-whole: 2 red tiles out of 5 total tiles = 2/5 = 40%!'
  }
];

export const RCRTRatioProportionUnit17Player: React.FC<RCRTRatioProportionUnit17PlayerProps> = ({
  conceptName = 'Concept 17.5: Ratio and Proportion',
  ...
  // Interactive Tile Builder State
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. A ratio compares part to part (like 2 red to 3 blue written 2:3). A proportion compares part to the whole total (like 2 red out of 5 total tiles written 2/5 or 40%).'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Lock the distinction: ratio = part-to-part, proportion = part-to-whole.'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "Ratio is part-to-part; proportion is part-to-whole!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your memory to work! Add 2 red tiles and 3 blue tiles to build a ratio of 2:3 and proportion of 2/5 = 40%!'
        );
      }
  ...
  const handleAddRed = () => {
    if (redTiles < 4) {
      ...
      setRedTiles(updated);
      safeNarrate(`Added Red tile (${updated}).`);
    }
  };
  ...
  const handleAddBlue = () => {
    if (blueTiles < 4) {
      ...
      safeNarrate(`Added Blue tile (${updated}).`);
    }
  };
  ...
  const handleConfirmProportion = () => {
    if (isTargetPattern) {
      ...
      safeNarrate('Proportion Confirmed! 2 red out of 5 total = 2/5 = 40%! Target matched!');
    }
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                ...
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>Ratio (Part-to-Part):</strong> Compares one part directly to another part using a colon (e.g., 2 red to 3 blue is <strong>2:3</strong>).<br/>
                  • <strong>Proportion (Part-to-Whole):</strong> Compares one part to the whole total as a fraction or percentage (e.g., 2 red out of 5 total is <strong>2/5 = 40%</strong>).
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  📊 Pattern Rule: 2 Red : 3 Blue (Ratio = 2:3 | Proportion Red = 2/5 = 40%)
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Lock the distinction (ratio = part-to-part, proportion = part-to-whole) in memory.
              </p>
            ...
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border-2 border-dashed border-[#9c6f1f]/40">
                <p className="text-sm font-black text-[#9c6f1f]">
                  "Ratio is part-to-part; proportion is part-to-whole!"
                </p>
              </div>
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('Ratio is part-to-part; proportion is part-to-whole!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active Tile Builder Test 🚀 ➔
              </button>
            </div>
          )}
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#9c6f1f] uppercase tracking-wider">
                  🎯 Step 4: Tile Pattern Canvas
                </span>
                <span className="text-xs font-bold text-[#16241f]">
                  Target: <strong className="text-[#9c6f1f]">2 Red : 3 Blue</strong>
                </span>
              </div>
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10 text-center space-y-3">
                <div className="flex justify-center gap-2 min-h-[56px] items-center p-3 bg-white rounded-xl border border-[#16241f]/10 shadow-inner">
                  ...
                  {redTiles === 0 && blueTiles === 0 && (
                    <span className="text-xs text-[#16241f]/40 italic">Tap buttons below to add tiles!</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-left pt-2 border-t border-[#16241f]/10">
                  <div className="p-2.5 bg-white rounded-lg border border-[#16241f]/10">
                    <span className="text-[10px] font-bold text-[#9c6f1f] uppercase block">Ratio (Red : Blue)</span>
                    <span className="text-sm font-black text-[#16241f]">
                      {redTiles} : {blueTiles} {isTargetPattern ? '✅ (2 : 3)' : ''}
                    </span>
                  </div>
                  <div className="p-2.5 bg-white rounded-lg border border-[#16241f]/10">
                    <span className="text-[10px] font-bold text-[#16241f]/60 uppercase block">Proportion of Red</span>
                    <span className="text-sm font-black text-[#16241f]">
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 Target Matched! 2 Red : 3 Blue | Red Proportion = 2/5 = 40%! Active recall 100% accurate!
                </div>
              )}
...
export default RCRTRatioProportionUnit17Player;
