...
export interface RCRTDivideUnitFractionsPlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'When sharing 1/2 of a chocolate bar equally among 3 people, what is the calculation and answer?',
    options: [
      '1/2 ÷ 3 = 1/6 (multiply denominator 2 by 3 to get 6)',
      '1/2 ÷ 3 = 3/2',
      '1/2 ÷ 3 = 1/5 (add 2 + 3)'
    ],
    correctIndex: 0,
    hint: 'To divide a unit fraction by a whole number, multiply the bottom number by the whole number: 2 × 3 = 6, so 1/6!'
  },
  {
    id: 2,
    questionText: 'If 1/4 metre of ribbon is cut into 2 equal pieces, how long is each piece as a fraction of a metre?',
    options: [
      '1/8 metre (since denominator 4 × 2 = 8)',
      '1/2 metre',
      '2/4 metre'
    ],
    correctIndex: 0,
    hint: 'Multiply the denominator 4 by 2 to get 8—each piece is 1/8 metre!'
  },
  {
    id: 3,
    questionText: 'Why does the numerator (top number) stay 1 when dividing a unit fraction by a whole number?',
    options: [
      'Because you are describing 1 small slice out of the new total number of equal pieces',
      'Because whole numbers cannot change fractions',
      'Because 1 is the largest possible fraction'
    ],
    correctIndex: 0,
    hint: 'You are splitting 1 slice into smaller pieces, so you are still looking at 1 of those smaller slices!'
  }
];

export const RCRTDivideUnitFractionsPlayer: React.FC<RCRTDivideUnitFractionsPlayerProps> = ({
  conceptName = 'Concept 17.3: Dividing Unit Fractions',
  ...
  // Interactive Test State for 1/2 ÷ 3
  const [dividedResult, setDividedResult] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. A unit fraction has a numerator of 1. To divide a unit fraction by a whole number, multiply the denominator by that whole number. The numerator always stays 1 because each new slice is smaller!'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Remember: multiply the bottom number by the whole number, top number stays 1.'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "Top stays 1, multiply the bottom by the whole number!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your memory to work! Share 1/2 of a chocolate bar equally among 3 people by dividing 1/2 by 3!'
        );
      }
      ...
      safeNarrate(
        'Congratulations! You mastered Dividing Unit Fractions with RCRT active recall and earned 3 stars!'
      );
    }
  ...
  const handleDivide = () => {
    setDividedResult('1/6');
    setTestCompleted(true);
    setStars((prev) => prev + 1);
    safeNarrate('Correct! 1/2 divided by 3 equals 1/6, because denominator 2 × 3 = 6! Target matched!');
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                <h3 className="font-bold text-base text-[#16241f] mb-1">
                  Rule: Dividing Unit Fractions by Whole Numbers
                </h3>
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>Rule:</strong> Multiply the denominator (bottom number) by the whole number.<br/>
                  • <strong>Numerator:</strong> Stays 1 because you are finding 1 smaller piece.<br/>
                  • <strong>Example 1:</strong> Share 1/2 chocolate bar among 3 people ➔ 1/2 ÷ 3 = 1/(2×3) = <strong>1/6</strong>.<br/>
                  • <strong>Example 2:</strong> 1/4 metre ribbon ÷ 2 ➔ 1/4 ÷ 2 = 1/(4×2) = <strong>1/8 metre</strong>.
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  ✂️ Division Rule: 1/2 ÷ 3 = 1 / (2 × 3) = 1/6
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Lock the unit fraction division rule in memory.
              </p>
            ...
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border-2 border-dashed border-[#9c6f1f]/40">
                <p className="text-sm font-black text-[#9c6f1f]">
                  "Top stays 1, multiply the bottom by the whole number!"
                </p>
              </div>
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('Top stays 1, multiply the bottom by the whole number!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active Slice Splitter Test 🚀 ➔
              </button>
            </div>
          )}
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#9c6f1f] uppercase tracking-wider">
                  🎯 Step 4: Interactive Slice Splitter
                </span>
                <span className="text-xs font-bold text-[#16241f]">
                  Task: Divide <strong className="text-[#9c6f1f]">1/2 ÷ 3</strong>
                </span>
              </div>
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10 text-center space-y-4">
                <div className="text-lg font-black text-[#16241f]">
                  Chocolate Bar Problem: Share <span className="text-[#9c6f1f]">1/2</span> bar among <span className="text-[#16241f]">3 people</span>
                </div>

                <div className={`p-4 rounded-xl border-2 transition-all ${dividedResult !== null ? 'bg-emerald-50 border-emerald-500 text-emerald-900' : 'bg-white border-[#16241f]/20'}`}>
                  {dividedResult === null ? (
                    <div>
                      <p className="text-xs text-[#16241f] mb-3 font-bold">
                        Multiply the denominator (2) by 3 people:
                      </p>
                      <button
                      ...
                      >
                        ➗ Tap to Multiply Denominator: 2 × 3 ➔ 1/6
                      </button>
                    </div>
                  ) : (
                    <div className="text-base font-black">
                      🎉 1/2 ÷ 3 = <span className="text-emerald-700">1/6</span> of the whole bar!
                    </div>
                  )}
                </div>
              </div>
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 Target Matched! 1/2 ÷ 3 = 1/6! Active recall 100% accurate!
                </div>
              )}
      ...
      {phase === 'passed' && (
        <div className="text-center py-8 space-y-5 animate-fade-in">
          ...
          <p className="text-xs text-[#16241f]/80 max-w-md mx-auto leading-relaxed">
            By Reading, Covering, Reciting, and Testing without looking, you built permanent memory for Dividing Unit Fractions!
          </p>
...
export default RCRTDivideUnitFractionsPlayer;
