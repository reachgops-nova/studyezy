...
export interface RCRTWorldTimeZonesPlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'If it is 12:00 noon in Lagos and Delhi is 5 hours ahead (east), what time is it in Delhi?',
    options: [
      '17:00 (5:00 pm) — because 12:00 + 5 hours = 17:00',
      '07:00 (7:00 am)',
      '12:00 noon (exact same time)'
    ],
    correctIndex: 0,
    hint: 'East is ahead! Add the 5 hours difference to the Lagos starting time (12 + 5 = 17:00).'
  },
  {
    id: 2,
    questionText: 'When finding local time in a zone that is WEST (behind in time), what calculation rule do you use?',
    options: [
      'Subtract the hour difference from the starting time (which can shift the date to the previous day)',
      'Add the hour difference to the starting time',
      'Multiply the starting time by 2'
    ],
    correctIndex: 0,
    hint: 'West is behind! Subtract hours as you move west, and remember this can shift to the previous day.'
  },
  {
    id: 3,
    questionText: 'If it is 7:00 am on 13 July in Sydney, and a city is 14 hours behind Sydney, what is the local time and date there?',
    options: [
      '5:00 pm on 12 July (the previous day)',
      '9:00 pm on 13 July',
      '7:00 am on 14 July'
    ],
    correctIndex: 0,
    hint: 'Count back 7 hours to 12:00 midnight (13 July start), then back 7 more hours into 12 July to reach 5:00 pm!'
  }
];

export const RCRTWorldTimeZonesPlayer: React.FC<RCRTWorldTimeZonesPlayerProps> = ({
  conceptName = 'Concept 18.1: World Time Zones',
  ...
  // Interactive Test State (Lagos 12:00 -> Delhi +5 hrs EAST -> Target 17:00)
  const [currentHour, setCurrentHour] = useState<number>(12);
  const [targetHour] = useState<number>(17);
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. The world is divided into 24 hourly time zones. Moving East means time is ahead, so add hours. Moving West means time is behind, so subtract hours. If Lagos is 12:00 noon and Delhi is 5 hours East, add 5 hours to get 17:00 (5:00 pm)!'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Remember: East = ahead (add), West = behind (subtract).'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "East is ahead, add the hours! West is behind, subtract the hours!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your memory to work! Starting at Lagos time 12:00 noon, add 5 hours for Delhi (5 hours East) to reach 17:00!'
        );
      }
      ...
      safeNarrate(
        'Congratulations! You mastered World Time Zones with RCRT active recall and earned 3 stars!'
      );
    }
  ...
  const handleAddHour = (hours: number) => {
    const nextH = currentHour + hours;
    setCurrentHour(nextH);
    if (nextH === targetHour) {
      setTestCompleted(true);
      setStars((prev) => prev + 1);
      safeNarrate('Target Matched! 12:00 + 5 hours = 17:00 (5:00 pm in Delhi)! Active recall 100% accurate!');
    } else {
      safeNarrate(`Current clock time: ${nextH}:00. Target is ${targetHour}:00.`);
    }
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                <h3 className="font-bold text-base text-[#16241f] mb-1">
                  Rule: World Time Zones Calculation
                </h3>
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>East = Ahead (ADD):</strong> Add hours when moving east to zones that are ahead.<br/>
                  • <strong>West = Behind (SUBTRACT):</strong> Subtract hours when moving west to zones that are behind.<br/>
                  • <strong>Example:</strong> Lagos = 12:00 noon. Delhi is 5 hours ahead (East) ➔ 12:00 + 5 hours = <strong>17:00 (5:00 pm)</strong>.
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  🌍 Memory Rule: East = Ahead (Add) | West = Behind (Subtract)
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Lock the East = Add, West = Subtract rule in memory.
              </p>
            ...
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border-2 border-dashed border-[#9c6f1f]/40">
                <p className="text-sm font-black text-[#9c6f1f]">
                  "East is ahead, add the hours! West is behind, subtract the hours!"
                </p>
              </div>
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('East is ahead, add the hours! West is behind, subtract the hours!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active World Clock Test 🚀 ➔
              </button>
            </div>
          )}
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#9c6f1f] uppercase tracking-wider">
                  🎯 Step 4: Time Zone Converter
                </span>
                <span className="text-xs font-bold text-[#16241f]">
                  Target: Delhi Time <strong className="text-[#9c6f1f]">(17:00 / 5:00 pm)</strong>
                </span>
              </div>
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10 text-center space-y-4">
                <div className="text-xs font-bold text-[#16241f]">
                  Lagos Start Time: <span className="text-base font-black">12:00 (Noon)</span> ➔ Delhi is <span className="text-[#9c6f1f] font-black">5 Hours East (Ahead)</span>
                </div>

                <div className="p-4 bg-white rounded-xl border border-[#16241f]/10 shadow-inner max-w-xs mx-auto">
                  <span className="text-xs text-[#16241f]/60 uppercase font-bold block mb-1">Delhi Local Clock</span>
                  <span className="text-3xl font-black text-[#16241f]">
                    {currentHour}:00 {currentHour >= 12 ? 'pm' : 'am'}
                  </span>
                </div>
                ...
                <div className="flex justify-center gap-2">
                  <button
                    ...
                    disabled={currentHour >= targetHour}
                    className="py-2.5 px-4 bg-[#9c6f1f] text-white font-bold text-xs rounded-xl shadow hover:bg-[#9c6f1f]/90 disabled:opacity-40"
                  >
                    ➕ Add +1 Hour
                  </button>
                  <button
                    onClick={() => handleAddHour(5)}
                    disabled={currentHour >= targetHour}
                    className="py-2.5 px-4 bg-[#16241f] text-white font-bold text-xs rounded-xl shadow hover:bg-[#16241f]/90 disabled:opacity-40"
                  >
                    ⏩ Jump +5 Hours
                  </button>
                </div>
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 Target Matched! 12:00 + 5 hours = 17:00 (5:00 pm)! Active recall 100% accurate!
                </div>
              )}
      ...
      {phase === 'passed' && (
        <div className="text-center py-8 space-y-5 animate-fade-in">
          ...
          <p className="text-xs text-[#16241f]/80 max-w-md mx-auto leading-relaxed">
            By Reading, Covering, Reciting, and Testing without looking, you built permanent memory for World Time Zones!
          </p>
...
export default RCRTWorldTimeZonesPlayer;
