...
export interface RCRTFactOpinionEnglishPlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'What is the key difference between a fact and an opinion?',
    options: [
      'A fact can be proven true or false with evidence; an opinion is a personal belief or judgement',
      'A fact is always written in gold ink',
      'An opinion can be checked using a microscope'
    ],
    correctIndex: 0,
    hint: 'Facts have provable evidence; opinions express personal feelings or beliefs!'
  },
  {
    id: 2,
    questionText: 'Which signal words often indicate that a sentence is an OPINION?',
    options: [
      'Words like "best", "worst", "beautiful", and "should"',
      'Words like "number", "date", and "measured"',
      'Capital letters and question marks'
    ],
    correctIndex: 0,
    hint: 'Words like "best", "worst", and "should" express personal judgements rather than provable facts!'
  },
  {
    id: 3,
    questionText: 'Is the sentence "The fable was published in 2021" a fact or an opinion?',
    options: [
      'A Fact — because the publication year can be checked and proven using records',
      'An Opinion — because people might dislike the year 2021',
      'Neither a fact nor an opinion'
    ],
    correctIndex: 0,
    hint: 'A publication year is provable evidence, so it is a Fact!'
  }
];

interface FactOpinionCard {
  id: string;
  text: string;
  isFact: boolean;
}

const FACT_OPINION_CARDS: FactOpinionCard[] = [
  { id: '1', text: '🌍 "The story is set in a small mountain village."', isFact: true },
  { id: '2', text: '🏆 "This is the best fable ever written in history!"', isFact: false },
  { id: '3', text: '📚 "Fables feature animal characters with human traits."', isFact: true },
  { id: '4', text: '⭐ "Everyone should read this fable every morning."', isFact: false }
];

export const RCRTFactOpinionEnglishPlayer: React.FC<RCRTFactOpinionEnglishPlayerProps> = ({
  conceptName = 'Concept 1.7: Fact vs. Opinion',
  ...
  const [opinionList, setOpinionList] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. A fact is something that can be proven true or false with evidence. An opinion is a personal belief or judgement that cannot be proven. Signal words like "best", "worst", and "should" indicate an opinion!'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Remember: Facts = Provable Evidence; Opinions = Personal Beliefs & Signal Words like "best" or "should".'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "Facts can be proven true or false; opinions are personal beliefs!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your critical thinking eyes on! Tap each card to classify it as a Fact or an Opinion!'
        );
      }
      ...
      safeNarrate(
        'Congratulations! You mastered Fact vs. Opinion with RCRT active recall and earned 3 stars!'
      );
    }
  ...
  const handleClassify = (card: FactOpinionCard, category: 'fact' | 'opinion') => {
    if (factList.includes(card.id) || opinionList.includes(card.id)) return;

    if (category === 'fact' && card.isFact) {
      const nextFact = [...factList, card.id];
      setFactList(nextFact);
      safeNarrate(`Correct! "${card.text}" can be proven with evidence, so it is a Fact.`);
      checkTestCompletion(nextFact, opinionList);
    } else if (category === 'opinion' && !card.isFact) {
      const nextOpinion = [...opinionList, card.id];
      setOpinionList(nextOpinion);
      safeNarrate(`Correct! "${card.text}" expresses a personal belief, so it is an Opinion.`);
      checkTestCompletion(factList, nextOpinion);
    } else {
      safeNarrate('Try again! Look for signal words like "best" or "should" for opinions, or provable details for facts.');
    }
  };

  const checkTestCompletion = (facts: string[], opinions: string[]) => {
    if (facts.length === 2 && opinions.length === 2) {
      setTestCompleted(true);
      setStars((prev) => prev + 1);
      safeNarrate('Outstanding! You correctly classified all facts and opinions!');
    }
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                <h3 className="font-bold text-base text-[#16241f] mb-1">
                  Rule: Fact vs. Opinion
                </h3>
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>Fact:</strong> Can be proven true or false using evidence.<br/>
                  • <strong>Opinion:</strong> Personal belief or judgement—cannot be proven.<br/>
                  • <strong>Signal Words:</strong> Words like "best", "worst", "should", "beautiful" indicate opinions!
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  ⚖️ Fact Rule: "Set in a village" (Fact) vs. "Best fable ever" (Opinion with "best")
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Remember: facts are provable; opinions use signal words like "best" or "should".
              </p>
              <div className="p-3 bg-white/10 rounded-xl text-xs font-mono text-amber-200">
                🔒 Fact/Opinion rules stored in memory...
              </div>
              <button
                onClick={handleNextRcrtStep}
          ...
          {rcrtStep === 'RECITE' && (
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('Facts can be proven true or false; opinions are personal beliefs!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active Statement Classifier Test 🚀 ➔
              </button>
            </div>
          )}
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-[#16241f]">
                  Progress: {factList.length + opinionList.length} / 4
                </span>
              </div>

              <div className="space-y-3">
                {FACT_OPINION_CARDS.map((card) => {
                  const isFact = factList.includes(card.id);
                  const isOpinion = opinionList.includes(card.id);
                  const isDone = isFact || isOpinion;
                    ...
                    >
                      ...
                      {!isDone ? (
                        ...
                        <div className="text-[11px] font-black text-emerald-700">
                          {isFact ? '📊 Classified as FACT (Provable Evidence)' : '💭 Classified as OPINION (Personal Belief)'}
                        </div>
                      )}
                    </div>
                  );
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 All Facts & Opinions Classified Correctly! Active recall 100% accurate!
                </div>
              )}
      ...
      {phase === 'passed' && (
        <div className="text-center py-8 space-y-5 animate-fade-in">
          ...
          <p className="text-xs text-[#16241f]/80 max-w-md mx-auto leading-relaxed">
            By Reading, Covering, Reciting, and Testing without looking, you built permanent memory for Fact vs. Opinion!
          </p>
...
export default RCRTFactOpinionEnglishPlayer;
