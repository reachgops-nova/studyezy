...
export interface RCRTCompareOrderFDPPlayerProps {
  conceptName?: string;
  unitTitle?: string;
  onSuccess?: () => void;
...
const QUIZ_QUESTIONS: Question[] = [
  {
    id: 1,
    questionText: 'When comparing 0.7 and 6/10, which number is larger and why?',
    options: [
      '0.7 is larger because 0.7 equals 7/10, which is greater than 6/10',
      '6/10 is larger because 10 is a bigger number',
      'They are exactly equal'
    ],
    correctIndex: 0,
    hint: 'Convert 0.7 to a fraction (7/10) or 6/10 to a decimal (0.6) before comparing!'
  },
  {
    id: 2,
    questionText: 'What is the correct order of 40%, 6/10, and 0.7 from least to greatest?',
    options: [
      '40% < 6/10 < 0.7 (since 0.4 < 0.6 < 0.7)',
      '0.7 < 6/10 < 40%',
      '6/10 < 40% < 0.7'
    ],
    correctIndex: 0,
    hint: 'Convert all three to decimals: 40% = 0.4, 6/10 = 0.6, and 0.7 stays 0.7!'
  },
  {
    id: 3,
    questionText: 'What is the best general rule for comparing mixed forms (fractions, decimals, percentages)?',
    options: [
      'Convert all values to the same form (all decimals or all percentages) first',
      'Always pick the fraction with the biggest denominator',
      'Only compare the first digit after the decimal point'
    ],
    correctIndex: 0,
    hint: 'Always dress all numbers in matching clothes—convert everything to percentages or decimals first!'
  }
];

export const RCRTCompareOrderFDPPlayer: React.FC<RCRTCompareOrderFDPPlayerProps> = ({
  conceptName = 'Concept 17.1: Comparing & Ordering Fractions, Decimals, Percentages',
  ...
  useEffect(() => {
    if (phase === 'rcrt_loop') {
      if (rcrtStep === 'READ') {
        safeNarrate(
          'STEP 1: READ. Fractions, decimals, and percentages express parts of a whole. To compare values in different forms, convert everything to the same form—like all decimals or all percentages—first! For example, 0.7 equals 7/10, so 0.7 is greater than 6/10.'
        );
      } else if (rcrtStep === 'COVER') {
        safeNarrate(
          'STEP 2: COVER. The rule card is now hidden! Do not look back. Remember to convert all numbers to decimals or percentages first.'
        );
      } else if (rcrtStep === 'RECITE') {
        safeNarrate(
          'STEP 3: RECITE. Say the rule aloud: "Dress all numbers in matching clothes—convert to decimals or percentages first!" Tap the button when ready.'
        );
      } else if (rcrtStep === 'TEST') {
        safeNarrate(
          'STEP 4: TEST! Put your memory to work! Convert 40%, 6/10, and 0.7 into decimals, then tap them in order from smallest to largest!'
        );
      }
      ...
      safeNarrate(
        'Congratulations! You mastered Comparing and Ordering Fractions, Decimals, and Percentages with RCRT active recall and earned 3 stars!'
      );
    }
  ...
  const handleConvert = () => {
    setConverted(true);
    safeNarrate('Converted! 40% becomes 0.4, 6/10 becomes 0.6, and 0.7 stays 0.7. Now tap them from smallest to largest!');
  };

  const handleCardTap = (val: string) => {
    if (!converted || sortedOrder.includes(val)) return;
    const nextOrder = [...sortedOrder, val];
    setSortedOrder(nextOrder);

    if (nextOrder.length === 3) {
      if (nextOrder[0] === '40%' && nextOrder[1] === '6/10' && nextOrder[2] === '0.7') {
        setTestCompleted(true);
        setStars((prev) => prev + 1);
        safeNarrate('Perfect order! 40% (0.4) < 6/10 (0.6) < 0.7! Your active recall was 100% accurate!');
      } else {
        safeNarrate('Incorrect order. Remember: 0.4 comes first, then 0.6, then 0.7. Resetting order!');
        setTimeout(() => setSortedOrder([]), 1200);
      }
    }
  };
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10">
                <h3 className="font-bold text-base text-[#16241f] mb-1">
                  Rule: Convert to Same Form Before Comparing
                </h3>
                <p className="text-xs text-[#16241f] leading-relaxed">
                  • <strong>Matching Forms:</strong> To compare values in different forms (fractions, decimals, percentages), convert everything to percentages or decimals first.<br/>
                  • <strong>Example:</strong> 0.7 = 7/10, which is greater than 6/10.<br/>
                  • <strong>Ordering Example:</strong> To order 40%, 6/10, and 0.7: convert to decimals (0.4, 0.6, 0.7) ➔ <strong>40% &lt; 6/10 &lt; 0.7</strong>.
                </p>
                <div className="mt-3 p-3 bg-white rounded-lg text-center font-bold text-xs text-[#9c6f1f]">
                  📊 Conversion Strategy: 40% (0.4) &lt; 6/10 (0.6) &lt; 0.7 (0.7)
                </div>
              </div>
          ...
          {rcrtStep === 'COVER' && (
            <div className="p-8 bg-[#16241f] text-white rounded-2xl shadow-xl text-center space-y-4 animate-shake">
              ...
              <p className="text-xs text-[#f4f6f1]/80 max-w-sm mx-auto leading-relaxed">
                The rule card is now hidden from view. No peeking! Lock the conversion strategy in working memory.
              </p>
            ...
            <div className="p-6 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm text-center space-y-5">
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border-2 border-dashed border-[#9c6f1f]/40">
                <p className="text-sm font-black text-[#9c6f1f]">
                  "Dress all numbers in matching clothes—convert to decimals or percentages first!"
                </p>
              </div>
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={() => {
                    ...
                    safeNarrate('Dress all numbers in matching clothes—convert to decimals or percentages first!');
                  }}
              ...
              <button
                ...
                className="w-full py-3 bg-[#16241f] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow hover:bg-[#16241f]/90"
              >
                Ready for Step 4: Active Converter & Sorter Test 🚀 ➔
              </button>
            </div>
          )}
            ...
            <div className="p-5 bg-white rounded-2xl border-2 border-[#16241f]/15 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#9c6f1f] uppercase tracking-wider">
                  🎯 Step 4: Convert & Order Cards
                </span>
                <span className="text-xs font-bold text-[#16241f]">
                  Order: <strong className="text-[#9c6f1f]">Smallest ➔ Largest</strong>
                </span>
              </div>
              ...
              <div className="p-4 bg-[#f4f6f1] rounded-xl border border-[#16241f]/10 text-center space-y-4">
                {!converted ? (
                  <div>
                    <p className="text-xs font-bold text-[#16241f] mb-3">
                      Three cards are in different forms: <strong>40%</strong>, <strong>6/10</strong>, and <strong>0.7</strong>. Tap below to convert them to decimals!
                    </p>
                    <button
                    ...
                    >
                      🔄 Convert Cards to Decimals (Matching Clothes)
                    </button>
                  </div>
                ) : (
                  <div>
                    <p className="text-xs font-bold text-[#16241f] mb-3">
                      Tap the cards in order from <strong>smallest to largest</strong>:
                    </p>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ...
                        const isSelected = sortedOrder.includes(card.raw);
                        const orderIdx = sortedOrder.indexOf(card.raw);
                        return (
                          <button
                            key={card.raw}
                            onClick={() => handleCardTap(card.raw)}
                            className={`p-3 rounded-xl border-2 font-bold transition-all text-center ${
                              isSelected
                                ? 'bg-[#16241f] text-white border-[#16241f] shadow-md'
                                : 'bg-white text-[#16241f] border-[#16241f]/20 hover:border-[#9c6f1f]'
                            }`}
                          >
                            <div className="text-base font-black">{card.raw}</div>
                            <div className="text-[10px] opacity-80 mt-0.5">= {card.dec}</div>
                            {isSelected && (
                              <span className="inline-block mt-1 text-[10px] bg-[#9c6f1f] text-white px-2 py-0.5 rounded-full">
                                #{orderIdx + 1}
                              </span>
                            )}
                          </button>
              ...
              {testCompleted && (
                <div className="p-3 bg-emerald-500/10 text-emerald-800 rounded-xl text-xs font-bold text-center border border-emerald-500/30 animate-bounce">
                  🎉 Correct Order! 40% (0.4) &lt; 6/10 (0.6) &lt; 0.7! Active recall 100% accurate!
                </div>
              )}
      ...
      {phase === 'passed' && (
        <div className="text-center py-8 space-y-5 animate-fade-in">
          ...
          <p className="text-xs text-[#16241f]/80 max-w-md mx-auto leading-relaxed">
            By Reading, Covering, Reciting, and Testing without looking, you built permanent memory for Comparing and Ordering Fractions, Decimals, and Percentages!
          </p>
...
export default RCRTCompareOrderFDPPlayer;
